                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <a href="{{route('admin.dataanak.detailanak', $item->id_anak)}}" class="btn btn-success btn-sm" ><span class="fa fa-id-card"></span></a>
                                                        <button class="btn btn-danger btn-sm" onclick="hapusData({{ $item->id_anak }})"><span class="fa fa-trash"></span></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                ...
                <div class="form-group ">
                    ...
                    <div class="input-group">
                        ...
                        <select name="dataibu" id= "dataibu" required class="form-control select2bs4">
                            ...
                            @foreach ($Ibu as $item)
                                <option value="{{ $item->id_ibu }}">{{ $item->nama_ibu }} - {{ $item->nama_suami }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group ">
                            ...
                            <div class="input-group">
                                ...
                                <select name="dataibu" id= "dataibuupdate" required class="form-control select2bs4">
                                    <option value="">-- Pilih Data Orangtua --</option>
                                    @foreach ($Ibu as $item)
                                        <option value="{{ $item->id_ibu }}">{{ $item->nama_ibu }} - {{ $item->nik }}</option>
                                    @endforeach
                                </select>
                            </div>
...
<script>
  ...
  theme: 'bootstrap4',
        ...
        $('#formInsert').on('submit', (function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.dataanak.insert') }}",
        ...
        $('#formUpdate').on('submit', (function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.dataanak.update') }}",
                headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                data: formData,
                contentType: false,
                processData: false,
    ...
    function hapusData(id) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    method: 'DELETE',
                    url: "{{ route('admin.dataanak.delete') }}",
                    headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                    data: {
                        id
                    },
    ...
    function ubahData(item) {
        ...
        $('#dataibuupdate').val(item.id_ibu).trigger('change');
