@extends('admin.layout.index', ['title' => 'Data Bidan'])
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <button class="float-right btn btn-primary btn-sm" data-toggle="modal" data-target="#addModal" data-backdrop="static" data-keyboard="false"><span class="fa fa-plus"></span> Tambah Data Bidan</button>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach ($list as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->no_hp }}</td>
                                                <td>{{ $item->alamat}} {{ $item->User->alamat}}</td>
                                                {{ $item->User->alamat}}
                                                <td>
                                                    <button class="btn btn-primary btn-sm" onclick="ubahData({{ $item }})"><span class="fa fa-pencil-alt"></span></button>
                                                    <button class="btn btn-danger btn-sm" onclick="hapusData({{ $item->id_bidan }})"><span class="fa fa-trash"></span></button>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
            ...
            <form action="javascript:void(0)" id="formInsert" method="POST">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addModalLabel">Tambah Data Bidan</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="name"><span class="text-danger">*</span> Nama Bidan</label>
                            <input type="text" class="form-control" id="name" name="name" required placeholder="Masukkan nama Bidan">
                        </div>
                        <div class="form-group">
            ...
            <form action="javascript:void(0)" id="formUpdate" method="POST">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="name"><span class="text-danger">*</span> Nama Bidan</label>
                            <input type="text" class="form-control" id="nameupdate" name="name"  placeholder="Masukkan nama Bidan">
                        </div>
                        <div class="form-group">
                            <label for="no_hp"><span class="text-danger">*</span>No. Hp</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#formInsert').on('submit', (function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.databidan.insert') }}",
            ...
            $('#formUpdate').on('submit', (function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.databidan.update') }}",
                    headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                    data: formData,
                    contentType: false,
                    processData: false,
        ...
        function hapusData(id) {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        method: 'DELETE',
                        url: "{{ route('admin.databidan.delete') }}",
                        headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                        data: {
                            id
                        },
        ...
        function ubahData(item) {
            $('#id').val(item.id_bidan);
            $('#nipupdate').val(item.nip);
            $('#nameupdate').val(item.nama_bidan);
